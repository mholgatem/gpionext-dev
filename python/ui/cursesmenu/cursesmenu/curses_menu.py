...
class CursesMenu(object):
	...
	def draw(self):
		...
		for index, item in enumerate(self.items):
			...
			while end < len( itemText ) - 1:
				totalLineCount += 1
				start = end + 1
				end = min(start + screen_cols - 15 - indent, len( itemText ) )
				if end < len( itemText ):
					for x in range(15):
						if itemText[end] in '- ,;':
							break
						end = min( end + 1, len( itemText ) - 1 )
				self.screen.addstr(5 + totalLineCount, 4 + indent, item.show(index)[ start: end ].lstrip(), text_style)
				indent = 16

		
		top_row = 0
		if 6 + totalLineCount > screen_rows:
			if screen_rows + self.current_option < 6 + totalLineCount:
				top_row = self.current_option
			else:
				top_row = 6 + totalLineCount - screen_rows
...
class MenuItem(object):
	...
	def __init__(self, text, menu=None, should_exit=False, maxWidth = 50):
		"""
	...
	def show(self, index):
		# using override to only show text
